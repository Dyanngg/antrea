...
function run_test {
  ...
  if $coverage; then
      go test -v -timeout=70m antrea.io/antrea/test/e2e -provider=kind --logs-export-dir=$ANTREA_LOG_DIR --coverage --coverage-dir $ANTREA_COV_DIR --skip=$skiplist -run=TestAntreaPolicy/TestFQDNFlaky
  else
      go test -v -timeout=65m antrea.io/antrea/test/e2e -provider=kind --logs-export-dir=$ANTREA_LOG_DIR --skip=$skiplist -run=TestAntreaPolicy/TestFQDNFlaky
  fi
  $TESTBED_CMD destroy kind
}
