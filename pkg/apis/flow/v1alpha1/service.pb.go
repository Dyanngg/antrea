...
type GetFlowsResponse struct {
	...
	// Identifies the Flow Aggregator process this stream is served by.
	// Changes only across a Flow Aggregator restart, which also resets
	// sequence_number numbering; see ResumeToken.
	...
	// in flows, or counted in dropped_count. -1 on the very first response of a
	// stream that starts at the oldest flow currently held and the ring buffer
	// has never produced anything yet. Clients can store this together with
	// stream_epoch and pass both back as a ResumeToken to continue this stream
	// later from the next flow record that they have not yet received.
	...
	// Set on the first response of a stream that was given a ResumeToken whose
	// stream_epoch did not match this one's: the Flow Aggregator restarted, so
	// resume was not honored and flows start over from the oldest one
	// currently held, same as a stream that never asked to resume at all.
	//
	...
	// count instead.
	ResumeReset   bool `protobuf:"varint,5,opt,name=resume_reset,json=resumeReset,proto3" json:"resume_reset,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}
...
func (x *GetFlowsResponse) GetStreamEpoch() string {
	if x != nil {
		return x.StreamEpoch
	}
	return ""
...
func (x *GetFlowsResponse) GetSequenceNumber() int64 {
	if x != nil {
		return x.SequenceNumber
	}
	return 0
...
const file_pkg_apis_flow_v1alpha1_service_proto_rawDesc = "" +
	...
	"\x0fsequence_number\x18\x02 \x01(\x03R\x0esequenceNumber\"\xeb\x01\n" +
	...
	"\fstream_epoch\x18\x03 \x01(\tR\vstreamEpoch\x12'\n" +
	"\x0fsequence_number\x18\x04 \x01(\x03R\x0esequenceNumber\x12!\n" +
	"\fresume_reset\x18\x05 \x01(\bR\vresumeReset*s\n" +
...
var file_pkg_apis_flow_v1alpha1_service_proto_depIdxs = []int32{
	...
	6, // 10: antrea_io.antrea.pkg.apis.flow.v1alpha1.FlowStreamService.GetFlows:output_type -> antrea_io.antrea.pkg.apis.flow.v1alpha1.GetFlowsResponse
