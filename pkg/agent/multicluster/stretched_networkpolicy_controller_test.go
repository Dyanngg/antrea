...
func newStretchedNetworkPolicyController(t *testing.T) (*fakeStretchedNetworkPolicyController, func()) {
	clientset := fake.NewSimpleClientset()
	informerFactory := informers.NewSharedInformerFactory(clientset, 12*time.Hour)
	listOptions := func(options *metav1.ListOptions) {
		options.FieldSelector = fields.OneTermEqualSelector("spec.nodeName", "test-nodejkl").String()
	}
...
func TestStretchedNetworkPolicyControllerPodEvent(t *testing.T) {
	...
	c, closeFn := newStretchedNetworkPolicyController(t)
	defer closeFn()
	...
	go func() {
		...
		if err := waitForNSRealized(c.clientset, ns); err != nil {
		...
		if err := waitForPodRealized(c.clientset, pod); err != nil {
		...
		c.ofClient.EXPECT().UpdatePodClassifierFlowsWithNewLabelID(gomock.Any(), openflow.UnknownLabelIdentity, gomock.Any(), gomock.Any()).Times(1)
		...
		c.labelIdentityCache[labelIdentity1.Spec.Label] = labelIdentity1.Spec.ID
		c.interfaceStore.EXPECT().GetContainerInterfacesByPod(pod.Name, pod.Namespace).Return([]*interfacestore.InterfaceConfig{&interfaceConfig}).Times(1)
		c.clientset.CoreV1().Pods(pod.Namespace).Create(context.TODO(), pod, metav1.CreateOptions{})
		if err := waitForPodRealized(c.clientset, pod); err != nil {
			t.Errorf("error when waiting for Pod '%s/%s' to be realized: %v", pod.Namespace, pod.Name, err)
		}
		c.ofClient.EXPECT().UpdatePodClassifierFlowsWithNewLabelID(gomock.Any(), labelIdentity1.Spec.ID, gomock.Any(), gomock.Any()).Times(1)
		c.podUpdateChannel.Notify(toPodAddEvent(pod))
		c.processNextWorkItem()
		assert.Equal(t, map[types.NamespacedName]string{{Name: pod.Name, Namespace: pod.Namespace}: labelIdentity1.Spec.Label}, c.podToLabel)
		assert.Equal(t, map[string]podSet{labelIdentity1.Spec.Label: {types.NamespacedName{Name: pod.Name, Namespace: pod.Namespace}: struct{}{}}}, c.labelToPods)
		...
		// Update Pod label.
		c.labelIdentityCache[labelIdentity2.Spec.Label] = labelIdentity2.Spec.ID
		c.interfaceStore.EXPECT().GetContainerInterfacesByPod(pod.Name, pod.Namespace).Return([]*interfacestore.InterfaceConfig{&interfaceConfig}).Times(1)
		pod.Labels["foo"] = "bar2"
		c.clientset.CoreV1().Pods(pod.Namespace).Update(context.TODO(), pod, metav1.UpdateOptions{})
		c.ofClient.EXPECT().UpdatePodClassifierFlowsWithNewLabelID(gomock.Any(), labelIdentity2.Spec.ID, gomock.Any(), gomock.Any()).Times(1)
		c.processNextWorkItem()
...
func TestStretchedNetworkPolicyControllerNSEvent(t *testing.T) {
	...
	go func() {
		...
		c.labelIdentityCache[labelIdentity1.Spec.Label] = labelIdentity1.Spec.ID
		c.labelIdentityCache[labelIdentity2.Spec.Label] = labelIdentity2.Spec.ID
		c.labelIdentityCache[labelIdentity3.Spec.Label] = labelIdentity3.Spec.ID
		c.labelIdentityCache[labelIdentity4.Spec.Label] = labelIdentity4.Spec.ID
		c.clientset.CoreV1().Namespaces().Create(context.TODO(), ns, metav1.CreateOptions{})
		if err := waitForNSRealized(c.clientset, ns); err != nil {
			t.Errorf("error when waiting for Namespace '%s' to be realized: %v", ns.Name, err)
		}
		...
		if err := waitForPodRealized(c.clientset, pod1); err != nil {
		...
		if err := waitForPodRealized(c.clientset, pod2); err != nil {
		...
		c.interfaceStore.EXPECT().GetContainerInterfacesByPod(pod1.Name, pod1.Namespace).Return([]*interfacestore.InterfaceConfig{&interfaceConfig}).Times(1)
		c.interfaceStore.EXPECT().GetContainerInterfacesByPod(pod2.Name, pod2.Namespace).Return([]*interfacestore.InterfaceConfig{&interfaceConfig}).Times(1)
		c.ofClient.EXPECT().UpdatePodClassifierFlowsWithNewLabelID(gomock.Any(), labelIdentity3.Spec.ID, gomock.Any(), gomock.Any()).Times(1)
		c.ofClient.EXPECT().UpdatePodClassifierFlowsWithNewLabelID(gomock.Any(), labelIdentity4.Spec.ID, gomock.Any(), gomock.Any()).Times(1)
		c.processNextWorkItem()
		c.processNextWorkItem()
...
func TestStretchedNetworkPolicyControllerLabelIdentityEvent(t *testing.T) {
	...
	go func() {
		...
		c.interfaceStore.EXPECT().GetContainerInterfacesByPod(pod.Name, pod.Namespace).Return([]*interfacestore.InterfaceConfig{&interfaceConfig}).Times(1)
		c.ofClient.EXPECT().UpdatePodClassifierFlowsWithNewLabelID(gomock.Any(), labelIdentity.Spec.ID, gomock.Any(), gomock.Any()).Times(1)
		c.processNextWorkItem()
		assert.Equal(t, map[types.NamespacedName]string{{Name: pod.Name, Namespace: pod.Namespace}: labelIdentity.Spec.Label}, c.podToLabel)
		assert.Equal(t, map[string]podSet{labelIdentity.Spec.Label: {types.NamespacedName{Name: pod.Name, Namespace: pod.Namespace}: struct{}{}}}, c.labelToPods)
		assert.Equal(t, map[string]uint32{labelIdentity.Spec.Label: labelIdentity.Spec.ID}, c.labelIdentityCache)
		...
		c.interfaceStore.EXPECT().GetContainerInterfacesByPod(pod.Name, pod.Namespace).Return([]*interfacestore.InterfaceConfig{&interfaceConfig}).Times(1)
		c.ofClient.EXPECT().UpdatePodClassifierFlowsWithNewLabelID(gomock.Any(), uint32(2), gomock.Any(), gomock.Any()).Times(1)
		c.processNextWorkItem()
		assert.Equal(t, map[types.NamespacedName]string{{Name: pod.Name, Namespace: pod.Namespace}: labelIdentity.Spec.Label}, c.podToLabel)
		assert.Equal(t, map[string]podSet{labelIdentity.Spec.Label: {types.NamespacedName{Name: pod.Name, Namespace: pod.Namespace}: struct{}{}}}, c.labelToPods)
		assert.Equal(t, map[string]uint32{labelIdentity.Spec.Label: uint32(2)}, c.labelIdentityCache)
		...
		assert.Equal(t, map[types.NamespacedName]string{{Name: pod.Name, Namespace: pod.Namespace}: labelIdentity.Spec.Label}, c.podToLabel)
		assert.Equal(t, map[string]podSet{labelIdentity.Spec.Label: {types.NamespacedName{Name: pod.Name, Namespace: pod.Namespace}: struct{}{}}}, c.labelToPods)
		assert.Equal(t, map[string]uint32{}, c.labelIdentityCache)
	}()
	select {
	case <-time.After(5 * time.Second):
		t.Errorf("Test didn't finish in time")
...
func waitForPodRealized(clientset *fake.Clientset, pod *corev1.Pod) error {
	return wait.Poll(interval, timeout, func() (bool, error) {
		_, err := clientset.CoreV1().Pods(pod.Namespace).Get(context.TODO(), pod.Name, metav1.GetOptions{})
		if err != nil {
			return false, nil
		}
		return true, err
...
func waitForNSRealized(clientset *fake.Clientset, ns *corev1.Namespace) error {
	return wait.Poll(interval, timeout, func() (bool, error) {
