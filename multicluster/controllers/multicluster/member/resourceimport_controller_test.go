...
func TestStaleControllerNoRaceWithResourceImportReconciler(t *testing.T) {
	...
	c := multicluster.NewStaleResCleanupController(fakeClient, common.TestScheme, "default", mcReconciler, multicluster.MemberCluster, &staleReconcileMutex)
	r := newLabelIdentityResourceImportReconciler(fakeClient, scheme, &staleReconcileMutex, fakeClient, localClusterID, "default", ca)
	...
	go mgr.Run(stopCh)
	// The staleController should backoff at first and eventually execute the cleanup.
	go c.Run(stopCh)
